...
func (r *EntityRepository) FindByID(ctx context.Context, id int64) (*model.Entity, error) {
	const q = `
		SELECT id, name, is_tam_admin, is_tc_developer, is_device_admin, created_at
		FROM entities
		WHERE id = ?
		LIMIT 1
	`
	...
	if err := row.Scan(&entity.ID, &entity.Name, &entity.IsTAMAdmin, &entity.IsTCDeveloper, &entity.IsDeviceAdmin, &entity.CreatedAt); err != nil {
