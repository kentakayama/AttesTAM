...
func testTAMResolveTEEPMessage_AgentUpdate_OK(t *testing.T, success bool) {
	...
	if success {
		...
		require.Equal(t, 1, len(agentStatus.SuitManifests))
		encodedComponentID, err := cbor.Marshal(tcID)
		require.Nil(t, err)
		require.Equal(t, encodedComponentID, agentStatus.SuitManifests[0].TrustedComponentID)
		require.Equal(t, uint64(1), agentStatus.SuitManifests[0].SequenceNumber)
	} else {
		...
		require.Nil(t, err)
		require.NotNil(t, agentStatus)
		require.Equal(t, 0, len(agentStatus.SuitManifests))
	}
}
